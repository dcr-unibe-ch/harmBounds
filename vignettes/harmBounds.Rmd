...
The idea is that under the null hypothesis of no safety concern, safety events are expected to occur at a frequency proportional to the randomization ratio. We can therefore do one sample binomial exact tests on the proportion of events in the treatment arm. Evidence that this proportion higher than what would be expected from randomization would indicate a safety problem.
...
Instead of using an alpha for each test, we can also control the family-wise type I error rate 
  ...
  Refer to [Choosing the test-wise alpha](#sec-alpha) for more details.
...
- rdH1: the risk difference (treatment minus control), with 0 being the null scenario and numbers >0 indicating a safety problem. Here the control proportion (r0) and the total number of participants (n) have to be specified.
